...
@section('content')
    ...
    <section class="glass p-4 mb-4">
        ...
        <form id="createForm" class="row g-2">
            <div class="col-md-6">
                <input id="dest" type="url" class="form-control form-control-lg" placeholder="https://destino.com"
                    required>
            </div>
            <div class="col-md-3">
...
@push('scripts')
    ...
    <script type="module">
        ...
        async function loadLinks(url = '/links') {
            ...
            const searchTxt = q.value?.trim();
            const qParam = searchTxt ? (hasQueryInUrl ? `&q=${encodeURIComponent(searchTxt)}` :
                `?q=${encodeURIComponent(searchTxt)}`) : '';
            const endpoint = url + qParam;
        ...
        `;
            ...
            document.querySelectorAll('[data-action="deactivate"]').forEach(btn => {
                btn.onclick = async () => {
                    ...
                    try {
                        await apiFetch(`/links/${btn.dataset.id}`, {
                            method: 'DELETE'
                        });
                        showAlert('success', 'Desactivado');
                        loadLinks();
                    } catch (e) {
        ...
        function renderPager(metaOrLinks) {
            ...
            for (const l of links) {
                ...
                a.onclick = (e) => {
                    ...
                    // 1) Forzar URL absoluta a objeto URL (soporta tanto absoluta como relativa)
                    const u = new URL(l.url, location.origin);

                    // 2) Quedarnos con path+query y quitar el prefijo /api/v1 (apiFetch lo añade)
                    const relative = (u.pathname + u.search).replace(/^\/api\/v1/, '');

                    // 3) Llamar con ruta relativa
                    loadLinks(relative);
                };
        ...
        q.addEventListener('input', () => {
            ...
            window.__t = setTimeout(() => loadLinks(), 350);
        });
        ...
        loadLinks().catch(() => {
            showAlert('danger', 'Inicia sesión');
            window.location.href = '/login';
        });
    </script>
@endpush
